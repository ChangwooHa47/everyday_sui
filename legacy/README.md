...
새 루트 npm workspace/운영 Docker 이미지에 이 디렉터리를 포함하지 않는다. 이곳의 Spring 사본은 호환성 비교용이다. 현재 운영 Spring은 [apps/api/spring](../apps/api/spring)에 있고, 기존 README의 배포/환경 설명은 당시 기록이다. 현재 모노레포 실행에는 [루트 README](../README.md)를 사용한다.
