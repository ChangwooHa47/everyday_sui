# Dear Mine(구 everyday) × Blockthon 2026 — 캐릭터 마켓 기획 기준

2026-09-12 추가 합의: 선물은 NFT로 구현하고 내부 NFT 마켓을 구성한다. 현재 코드와 다음 버전의 상세 요구사항은 [제품 기획안](PRODUCT_PLAN.md)을 따른다. 2026-09-19에는 승인된 외부 NFT의 예치·SUI 구매·금고 선물·회수까지 로컬 구현했지만, 새 계약 배포와 실제 testnet 왕복은 아직 완료하지 않았다.
...
| P0 씬 3 | 다른 사용자의 기억은 따라오지 않고, 자동 기억에 한 번 동의한 사용자의 구조화된 기억은 별도 지원 환경에서 이어짐 |
| P1 씬 4 | 대화 맥락으로 선물을 제안하고 Move 정책이 지출을 제한하며 실제 상품과 채팅 영수증이 사용자에게 도착 |

Memory는 사용자별 자동 장기기억, Agency는 정책 안에서 집행하는 캐릭터 지출, Ownership은 이용권과 접근 권한에 대응한다. P0를 먼저 완성한다.
...
| 기억 | MemWal SDK: 사용자 계정·delegate 검증, 포괄 동의 뒤 구조화된 항목의 자동 저장·검색, 캐릭터별 namespace |
| DB·공통 계약 | PostgreSQL의 기존 `everyday`·`public` schema를 단일 API가 사용한다. `apps/api/migrations`의 SQL V1–V11을 변경하지 않고 V12로 자동 기억 매핑을 추가했다. 기존 적용 이력을 유지하며 공통 API DTO는 `packages/contracts`에 둔다. |
...
8. 사용자가 MemWal 계정과 delegate를 연결해 자동 기억에 한 번 동의한 뒤에만 저장한다. 대화별 팝업은 띄우지 않되 출처·시점·유형·상태를 사용자×캐릭터로 매핑하고, 비밀정보·낮은 신뢰도·일시적 대화는 제외한다. API 활용 중지는 DB 설정이며 온체인 위임 철회·분산 사본 삭제와 다르다. 중계 서버·AI가 처리하는 평문을 숨기거나 운영자도 절대 볼 수 없다고 약속하지 않는다.
...
| P1 선물 | Move 금고·allowlist·한도 집행은 과거 실제 거래로 확인. 내부/승인 외부 NFT의 직접 구매·금고 선물·복구·수신 설정은 로컬 코드와 자동 테스트로 구현 | 새 계약/정책/상품을 배포한 실제 NFT 구매·선물·회수, 채팅 UI 표현. 기본 비활성이며 기존 Listing 한도는 0. |
