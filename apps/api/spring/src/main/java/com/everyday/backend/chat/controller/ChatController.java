...
public class ChatController {
    ...
    public ApiResponse<ChatMessageResponse> sendMessage(
        ...
        Long userId = SecurityUtils.getCurrentUserId();
        return ApiResponse.ok(chatService.sendMessage(userId, characterId, request.content(), request.requestId()));
    }

    public record GreetingRequest(java.util.UUID requestId) {}

    @PostMapping("/greeting")
    public ApiResponse<ChatMessageResponse> greeting(@PathVariable Long characterId,
            @RequestBody(required = false) GreetingRequest input) {
        return ApiResponse.ok(chatService.greeting(SecurityUtils.getCurrentUserId(), characterId, input == null ? null : input.requestId()));
    }

    @GetMapping("/messages/history")
    public ApiResponse<List<ChatMessageResponse>> history(@PathVariable Long characterId) {
        return ApiResponse.ok(chatService.history(SecurityUtils.getCurrentUserId(), characterId));
    }
}
