...
public class PhotoService {
    ...
    public record Plan(String context, String reference, String concept, String soulId) {}

    public Plan plan(Long userId, Long characterId, GeneratePhotoRequest request) {
        Character character = getOwnedCharacter(userId, characterId);
        PhotoConcept concept = PhotoConcept.fromCodeOrLabel(request.concept()).orElse(PhotoConcept.CUSTOM);
        if (concept == PhotoConcept.CUSTOM && (request.customPrompt() == null || request.customPrompt().isBlank()))
            throw new CustomException(ErrorCode.INVALID_REQUEST);
        String soulId = character.getSoulId();
        if (soulId != null && !character.isSoulReady() && !imageClient.soulReady(soulId)) soulId = null;
        return new Plan(imageContext(character, concept, request.customPrompt()), character.getProfileImageUrl(), concept.getLabel(), soulId);
    }

    public record Rendered(String prompt, String url) {}

    @Transactional(propagation = org.springframework.transaction.annotation.Propagation.NOT_SUPPORTED)
    public Rendered render(Plan plan) {
        String prompt = refineContext(plan.context());
        var urls = imageClient.generateImages(prompt, plan.reference(), 1, plan.soulId());
        if (urls.size() != 1) throw new CustomException(ErrorCode.IMAGE_API_ERROR);
        return new Rendered(prompt, urls.getFirst());
    }

    private String refineImagePrompt(Character character, PhotoConcept concept, String customPrompt) {
        return refineContext(imageContext(character, concept, customPrompt));
    }

    private String refineContext(String context) {
        String systemPrompt = """
        ...
        return llmClient.generate(systemPrompt, List.of(LlmMessage.user(context))).trim();
    }

    private String imageContext(Character character, PhotoConcept concept, String customPrompt) {
