...
public class HiggsfieldImageClient implements ImageClient {
    ...
    public List<String> generateImages(String prompt, String referenceImageUrl, int count) {
        return generateImages(prompt, referenceImageUrl, count, null);
    }

    @Override
    public List<String> generateImages(String prompt, String referenceImageUrl, int count, String soulId) {
        requireConfigured();
        ...
        SoulParams params = new SoulParams(
                prompt, "1536x2048", "1080p", batchSize, true, imageReference, soulId, soulId == null ? null : 1.0);
    ...
    public String trainSoul(String referenceImageUrl) {
        ...
        try {
            ...
            // Registration is asynchronous; callers must check soulReady before treating this ID as trained.
            return response.id();
        } catch (RestClientException e) {
    ...
    @Override
    public boolean soulReady(String soulId) {
        requireConfigured();
        try {
            var result = restClient.get().uri("/v1/custom-references/{id}", soulId).headers(this::authHeaders)
                    .retrieve().body(CustomReferenceResponse.class);
            if (result == null || !soulId.equals(result.id())) throw new CustomException(ErrorCode.IMAGE_API_ERROR);
            if ("failed".equalsIgnoreCase(result.status())) throw new CustomException(ErrorCode.IMAGE_API_ERROR);
            return "completed".equalsIgnoreCase(result.status());
        } catch (RestClientException e) { throw new CustomException(ErrorCode.IMAGE_API_ERROR); }
    }

    private List<String> pollForResult(String jobSetId) {
        long deadline = System.nanoTime() + java.time.Duration.ofMinutes(10).toNanos();
        for (int attempt = 0; attempt < MAX_POLL_ATTEMPTS && System.nanoTime() < deadline; attempt++) {
    ...
    public void requireConfigured() {
        if (properties.apiKey() == null || properties.apiKey().isBlank() || properties.apiSecret() == null || properties.apiSecret().isBlank()) {
    ...
    private record SoulParams(
            ...
            @JsonProperty("custom_reference_id") String customReferenceId,
            @JsonProperty("custom_reference_strength") Double customReferenceStrength) {
    }
