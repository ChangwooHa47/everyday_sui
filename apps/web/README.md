...
- 사진 생성은 작업 ID로 상태를 조회한다. 같은 요청의 중복 차감을 막고 실패 처리된 작업의 포인트를 환불한다.
...
[비교 모드](BASELINE.md)는 `NEXT_PUBLIC_LEGACY_BASELINE=1`과 Spring fixture를 사용하며 운영에서 사용하지 않는다. [사진 개선 기록](docs/사진-생성-개선-리캡.md)과 [Soul 스타일 목록](docs/soul-스타일-전체목록.md)은 원본에서 가져온 과거 참고 자료다.
