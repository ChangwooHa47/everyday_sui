...
비교 모드는 지갑 인증 대신 데모 JWT 인증을 사용한다. [BaselineApplication](../../legacy/spring/src/test/java/com/everyday/backend/baseline/BaselineApplication.java)은 테스트 클래스패스에서만 실행되며 운영 JAR에는 포함되지 않는다. H2와 고정 AI·이미지 응답을 사용하므로 실제 공급자 호출이나 운영 PostgreSQL·Sui 연동을 증명하지 않는다.

Node 24, npm 11, Java 21이 준비된 환경에서 루트 기준으로 비교 서버를 실행할 수 있다.

```powershell
legacy/spring/gradlew.bat -p legacy/spring bootTestRun --no-daemon
```
...
[CI](../../.github/workflows/ci.yml) uses Node type checks, unit tests, container checks and the [PostgreSQL product integration suite](../../infra/test-product.mjs). Java and the historical comparison server are excluded. The AI, image and market provider responses in the integration suite are fixtures.
