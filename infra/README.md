...
| 파일 | 역할과 실행 범위 |
| --- | --- |
| [compose.yaml](compose.yaml) | PostgreSQL 17 + Fastify API + private Spring. 웹은 별도 개발 서버. DB/API만 로컬 호스트 5432/3001에 노출한다. |
| [Dockerfile.api](Dockerfile.api), [Dockerfile.web](Dockerfile.web), [Dockerfile.spring](Dockerfile.spring) | 모두 저장소 루트 context로 빌드하는 독립 서비스 이미지 |
