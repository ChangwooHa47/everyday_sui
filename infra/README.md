...
현재 저장소의 배포 구성은 웹 하나, Node와 Spring을 함께 실행하는 백엔드 하나, PostgreSQL이다. 기존 제품 기능과 Web3 SDK를 유지하면서 양방향 내부 주소·배포 설정을 한 백엔드로 모은다. 두 런타임의 CPU·메모리와 DB 연결은 남으므로 요금 절감을 보장하지 않는다. 2026-09-12 승인된 운영 전환으로 별도 Spring 서비스를 제거했으며 기존 웹/API 주소와 PostgreSQL을 유지했다.
